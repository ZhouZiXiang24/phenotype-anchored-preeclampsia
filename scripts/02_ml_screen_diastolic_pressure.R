# Machine-learning screening for diastolic pressure
...
# Select diastolic pressure as the regression outcome.
clinicalrow<-as.data.frame(inputclinical[2,])
...
hm<-Heatmap(as.matrix(result5), name = "RMSE",
            ...
            col = c("#CF394B","#6A303A","#122B3B"),  # Color scale.
...
pdf(file.path(figure_dir, "rmse_diastolic_pressure.pdf"), width = cellwidth * ncol(result5) + 5, height = cellheight * nrow(result5) * 0.45)
draw(hm)
invisible(dev.off())
