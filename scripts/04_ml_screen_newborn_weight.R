# Machine-learning screening for newborn weight
...
# Select newborn weight as the regression outcome.
clinicalrow<-as.data.frame(inputclinical[4,])
...
pdf(file.path(figure_dir, "rmse_newborn_weight.pdf"), width = cellwidth * ncol(result5) + 5, height = cellheight * nrow(result5) * 0.45)
draw(hm)
invisible(dev.off())
